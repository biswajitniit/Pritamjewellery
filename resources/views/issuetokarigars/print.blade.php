        ...
        <table style="width:100%; border-collapse:collapse;">
          ...
          <tr>
            ...
            <td style="padding:5px 6px;">
              @if(isset($issuetokarigaritems[0]))
                {{ $issuetokarigaritems[0]->kid ?? '-' }}
              @else
                -
              @endif
            </td>
      ...
      <td style="width:50%; vertical-align:top; border:1px solid #dcdcdc; padding:0;">
        <table style="width:100%; border-collapse:collapse;">
          <tr>
            ...
            <td style="border-bottom:1px solid #dcdcdc; padding:5px 6px;">
              {{ date('d/m/Y', strtotime($issuetokarigar->customerorder->jo_date)) }}
            </td>
          </tr>
          <tr>
            <th style="background:#f8f8f8; text-align:left; border-bottom:1px solid #dcdcdc; padding:5px 6px; font-weight:600;">
            ...
            <td style="padding:5px 6px;">
              {{ date('d/m/Y', strtotime($issuetokarigaritems[0]->delivery_date)) }}
            </td>
          </tr>
        </table>
      </td>
  ...
  <table>
    ...
    @forelse($issuetokarigaritems as $item)
      ...
      <tr>
        ...
        <td class="text-right">{{ number_format((float)$item->st_weight, 2) }}</td>
        <td class="text-right">{{ $item->qty }}</td>
        <td class="text-right">{{ number_format($rowTotalWt, 2) }}</td>
        <td class="text-right">{{ number_format((float)$item->min_weight, 2) }}</td>
        <td class="text-right">{{ number_format((float)$item->max_weight, 2) }}</td>
      </tr>
    @empty
    ...
    <tr class="total-row">
      ...
      <td class="text-right">{{ number_format($totalTotalWt, 2) }}</td>
      <td></td>
      <td></td>
    </tr>
