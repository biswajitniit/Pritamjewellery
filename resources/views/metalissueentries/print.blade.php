...
<head>
<meta charset="utf-8" />
<title>Delivery Challan - A5 Landscape</title>
...
.footer {
    ...
    left: 6mm;
    right: 6mm;
    bottom: 2mm; /* very close to bottom border */
    display: flex;
...
<div class="page">
    ...
    <table class="info-table">
        ...
        <tr>
            <td>
                <strong>Name:</strong> {{ $karigars->kname }}<br><br>
                <strong>Address:</strong> {{ $karigars->address }}<br><br>
                <strong>GSTIN:</strong> {{ $karigars->gstin }}<br><br>
                <strong>State code:</strong> {{ $karigars->statecode }}
            </td>

            <td>
                <strong>Voucher No:</strong> {{ $metalissueentries->voucher_no }}<br><br>
                <strong>Voucher Date:</strong> {{ date('d/m/Y', strtotime($metalissueentries->metal_issue_entries_date)) }}<br><br>
    ...
    <table class="items">
        ...
        <tbody>
            ...
            <tr class="remove-desc">
                <td class="col-desc">
                    <div class="desc-wrapper">
                        <div class="desc-left">
                            Standard Gold Bar<br>
                            <span class="desc-indent">for Job Work</span>
                        </div>
                        <div class="desc-right">Add: Alloy</div>
                    </div>
                </td>

                <td class="col-hsn">{{ $metal->metal_hsn }}</td>
                <td class="col-purity">{{ $metalpurities->purity }}</td>
                <td class="col-gross"></td>
                <td class="col-net">
                    {{ number_format($metalissueentries->weight, 3, '.', '') }}<br>
                    {{ number_format($metalissueentries->alloy_gm, 3, '.', '') }}
                </td>
            </tr>
            ...
            <tr class="total-row">
                ...
                <td style="text-align:right;">{{ number_format($metalissueentries->netweight_gm,3, '.', '') }}</td>
            </tr>
    ...
    <!-- FOOTER TOP LINE -->
    <div class="issued">
        ISSUED FOR JOBWORK – SAC : {{ $metal->metal_sac }} – HSN FOR ALLOY : {{ $metalCOPPER->metal_hsn }} / {{ $metalSILVER->metal_hsn }}
    </div>
