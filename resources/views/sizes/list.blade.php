          ...
          <div class="col-12 col-xl-12">
            <div class="card border-top border-3 border-danger rounded-0">
              <div class="card-header py-3 px-4">
                <h5 class="mb-0 text-danger">Size
                </h5>
                <div id="fixed-social">
                  @php
                      $permission = getUserMenuPermission(Auth::user()->id, 'uoms', 'permissions_add');
                  @endphp

                  @if ($permission && $permission->permissions_add == 1)
                  <div>
                      <a href="{{ route('sizes.create') }}">ADD</a>
                  </div>
                  @endif
                                            ...
                                            <thead>
                                                <tr>
                                                  <th><i class="fa fa-cog style_cog"></i></th>
                                                    <th scope="col">#</th>
                                                    <th scope="col">schar</th>
                                                    <th scope="col">Item Name</th>
                                                    <th scope="col">ssize</th>
                                              ...
                                              <tr>
                                                <td><div class="dropdown dd__">
                                                  ...
                                                  <ul class="dropdown-menu">
                                                    ...
                                                    @php
                                                        $permission_edit = getUserMenuPermission(Auth::user()->id, 'sizes', 'permissions_edit');
                                                    @endphp
                                                    @if ($permission_edit && $permission_edit->permissions_edit == 1)
                                                        <li><a class="dropdown-item" href="{{ route('sizes.edit',[$size->id]) }}"><i class="fa fa-pencil"></i> Edit</a></li>
                                                    @endif

                                                    <li><hr class="dropdown-divider"></hr></li>

                                                    @php
                                                        $permission_delete = getUserMenuPermission(Auth::user()->id, 'sizes', 'permissions_delete');
                                                    @endphp
                                                  ...
                                                  <td>{{$size->ssize}}</td>
                                                  <td>{{$size->pcode->code}}</td>
                                                  <td>
                                                    @if($size->is_active == 'Yes')
