          ...
          <div class="col-12 col-xl-12">
            <div class="card border-top border-3 border-danger rounded-0">
              <div class="card-header py-3 px-4">
                <h5 class="mb-0 text-danger">Stone
                </h5>
                <div id="fixed-social">

                  @php
                      $permission = getUserMenuPermission(Auth::user()->id, 'stones', 'permissions_add');
                  @endphp

                  @if ($permission && $permission->permissions_add == 1)
                  <div>
                      <a href="{{ route('stones.create') }}">ADD</a>
                  </div>
                  @endif
                                        ...
                                        <table class="table mb-0 table-striped">
                                            <thead>
                                                <tr>
                                                    ...
                                                    <th scope="col">#</th>
                                                    <th scope="col">Additional Charge ID</th>
                                              ...
                                              <tr>
                                                <td><div class="dropdown dd__">
                                                  ...
                                                  <ul class="dropdown-menu">
                                                    @php
                                                        $permission_edit = getUserMenuPermission(Auth::user()->id, 'stones', 'permissions_edit');
                                                    @endphp
                                                    @if ($permission_edit && $permission_edit->permissions_edit == 1)
                                                        <li><a class="dropdown-item" href="{{ route('stones.edit',[$stone->id]) }}"><i class="fa fa-pencil"></i> Edit</a></li>
                                                    @endif

                                                    <li><hr class="dropdown-divider"></hr></li>

                                                    @php
                                                        $permission_delete = getUserMenuPermission(Auth::user()->id, 'stones', 'permissions_delete');
                                                    @endphp
                                                  ...
                                                  <td>{{$stone->additional_charge_id}}</td>
                                                  <td>{{$stone->category}}</td>
                                                  <td>{{$stone->description}}</td>
                                                  <td>{{$stone->uom}}</td>
                                                  <td>
                                                    @if($stone->is_active == 'Yes')
