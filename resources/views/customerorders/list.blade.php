                ...
                <div class="card border-top border-3 border-danger rounded-0">
                    <div class="card-header py-3 px-4">
                        ...
                        <div id="fixed-social">
                            ...
                            @if($lastTempOrderId != 0)
                            <div>
                                <a href="{{ route('customerordertemps.show',[$lastTempOrderId]) }}">View Upload Orders</a>
                            </div>
                            @endif

                            <div>
                                <a href="{{ route('customerorders.create', ['type' => 'bulk']) }}">ADD Bulk Upload</a>
                            </div>
                            <div>
                                <a href="{{ route('customerorders.create', ['type' => 'manual']) }}">ADD Manual</a>
                            </div>
                            @endif
                            ...
                            <div class="card-body">
                                <div class="table-responsive-xxl">
                                    ...
                                    <table class="table mb-0 table-striped">
                                        ...
                                        <tbody>
                                            @php $count=1; @endphp @forelse($customerorders as $customerorder)
                                            <tr>
                                                <td>
                                                    <div class="dropdown dd__">
                                                        ...
                                                        <ul class="dropdown-menu">
                                                            ...
                                                            @php $permission_delete = getUserMenuPermission(Auth::user()->id, 'customerorders', 'permissions_delete'); @endphp @if ($permission_delete && $permission_delete->permissions_delete == 1)
                                                            <li>
                                                ...
                                                <td>{{$customerorder->jo_no}}</td>
                                                <td>{{$customerorder->customer->cust_name}} ({{$customerorder->customer->cid}})</td>
                                                <td>@if($customerorder->order_type == 'ManualUpload') Manual Upload @else Bulk Upload @endif</td>
                                                <td>{{ date('Y-m-d',strtotime($customerorder->created_at)) }}</td>
                                            </tr>
